...
# read in the dataset
d.agg <- read.table("/Users/ggloor/Documents/0_git/CoDa_microbiome_tutorial/data/barton_agg.tsv", sep="\t", header=T, row.names=1)
...
PC1.wt <- paste("PC1: ", round(pcx.wt$sdev[1]^2/sum(pcx.wt$sdev^2),3), sep="")
PC2.wt <- paste("PC2: ", round(pcx.wt$sdev[2]^2/sum(pcx.wt$sdev^2),3), sep="")

# plot
par(mfrow=c(1,2))
biplot(pcx.wt, var.axes=FALSE, scale=0,  xlab=PC1.wt, ylab=PC2.wt, cex=c(0.6,.05))
